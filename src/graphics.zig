...
pub const Texture = struct {
    ...
    pub fn init(width: u32, height: u32, format: u32, data: ?[]const u8, tex_type: u32, params: []const Param) Texture {
        return initOptions(width, height, format, format, data.ptr, gl.UNSIGNED_BYTE, tex_type, true, params);
    }
    ...
    pub fn initOptions(
        ...
        // zig fmt: off
        gl.texImage2D(
            tex_type, 0, @intCast(i32, internal_format),
            @intCast(i32, width), @intCast(i32, height), 0,
            data_format, data_type,
            if (data) |ptr| @ptrCast(*const anyopaque, ptr) else null,
        );
        // zig fmt: on
        if (gen_mipmaps) gl.generateMipmap(tex_type);
    ...
    pub fn deinit(self: Texture) Texture {
...
pub const Shader = struct {
    ...
    pub fn from_srcs(
        allocator: Allocator,
        name: []const u8,
        srcs: struct { vertex: []const u8, fragment: []const u8 },
    ) Shader {
        var shader = Shader{
            .vert_id = 0,
            .frag_id = 0,
...
pub const ShadowMap = struct {
    ...
    // call `deinit` to cleanup
    pub fn init(size: u32, allocator: Allocator) ShadowMap {
        return .{
            .size = size,
            .framebuffer = Framebuffer.init(size, size),
            .camera = undefined,
            .shader = Shader.from_srcs(allocator, "shadow", .{
                .vertex = shader_vert_src,
                .fragment = shader_frag_src,
            }),
        };
    }
